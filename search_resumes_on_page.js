...
(function() {
    ...
    chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
        ...
        } else if (request.action === "evaluateCandidates" && !searchProcessed) {
            // Set flag to prevent duplicate processing
            searchProcessed = true;
            
            // Handle the evaluate candidates action
            const criteria = request.criteria || '';
            console.log(`Evaluation initiated with criteria: "${criteria}"`);

            if (!criteria) {
                console.log("No evaluation criteria provided");
                sendResponse({status: "Error: No evaluation criteria provided"});
                return;
            }

            console.log("Evaluation criteria received:", criteria);
            // Call the searchResumesOnPage function with the criteria
            searchResumesOnPage(criteria);
            sendResponse({status: "Evaluation started with criteria: " + criteria});
        } else {
            ...
            for (let i = 0; i < links.length; i++) {
                ...
                try {
                    ...
                    updateStatus(statusContainer, i, links.length, 
                        `Downloaded: ${link.text} (${(pdfBlob.size / 1024).toFixed(2)} KB)`, 'success');
    ...
    function createStatusContainer(totalLinks) {
        ...
        `;
        header.textContent = `Downloading PDFs (0/${totalLinks})`;
    ...
    function updateStatus(container, current, total, message, status) {
        ...
        // Update header
        header.textContent = `Downloading PDFs (${current}/${total})`;
        ...
        statusItem.innerHTML = `<span style="color:${statusColor}">●</span> ${message}`;
